...
parser.add_argument("-p", "--parameter", help="method parameters to search for")
...
struct call_t {
    ...
    char clazz[80];
    char method[80];
    char arg1[80];              // Assuming the argument can be represented as a string
};
...
if args.parameter:
    program += prefix_template % ("arg1", args.parameter)

trace_template = """
int NAME(struct pt_regs *ctx) {
    u64 *depth, zero = 0, clazz = 0, method = 0, arg1 = 0 ;
    struct call_t data = {};
    ...
    bpf_probe_read_user(&data.arg1, sizeof(data.arg1), (void *)arg1);
...
def enable_arg_probe(probe_name, func_name, read_class, read_method, read_args, is_return):
    global program, trace_template, usdt
    depth = "*depth + 1" if not is_return else "*depth | (1ULL << 63)"
    update = "++(*depth);" if not is_return else "if (*depth) --(*depth);"
    filter_class = "if (!prefix_class(data.clazz)) { return 0; }" \
    ...
    filter_method = "if (!prefix_method(data.method)) { return 0; }" \
                   if args.method else ""
    filter_args = "if (!prefix_method(data.arg1)) { return 0; }" \
                   if args.parameter else ""

    program += trace_template.replace("NAME", func_name)                \
                             .replace("READ_CLASS", read_class)         \
                             .replace("READ_METHOD", read_method)       \
			     .replace("READ_ARG", read_args)		\
                             .replace("FILTER_CLASS", filter_class)     \
                             .replace("FILTER_METHOD", filter_method)   \
			     .replace("FILTER_ARGS", filter_args)	\
                             .replace("DEPTH", depth)                   \
                             .replace("UPDATE", update)
    usdt.enable_probe_or_bail(probe_name, func_name)
...
if language == "java":
    enable_arg_probe("method__entry", "java_entry",
                 "bpf_usdt_readarg(2, ctx, &clazz);",
                 "bpf_usdt_readarg(4, ctx, &method);",
                 "bpf_usdt_readarg(6, ctx, &arg1);",  # additional line to read first argument
                 is_return=False)

    enable_arg_probe("method__return", "java_return",
                 "bpf_usdt_readarg(2, ctx, &clazz);",
                 "bpf_usdt_readarg(4, ctx, &method);",
		 "bpf_usdt_readarg(6, ctx, &arg1);",
		 is_return=True)
...
print("Tracing method calls in %s process %d... Ctrl-C to quit." %
      (language, args.pid))
print("%-3s %-6s %-6s %-8s %-4s %s" % ("CPU", "PID", "TID", "TIME(us)", "METHOD", "PARAMETERS"))
